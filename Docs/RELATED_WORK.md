...
|---|-------|--------------|--------------------------|
> **Author correction, 2026-09-11:** row 1 was previously attributed to "Andreasen & Esterle". The arXiv record and the paper itself list **Martin Braas and Lukas Esterle**. The archived PDF filename (`RW01_andreasen_esterle_*.pdf`) still carries the old name; the file contents are correct.

| 1 | **Braas & Esterle** — *Fixed-Persona SLMs with Modular Memory: Scalable NPC Dialogue on Consumer Hardware* (arXiv:2511.10277) | **Closest prior work.** DistilGPT-2, TinyLlama-1.1B and Mistral-7B + LoRA personas + runtime-swappable memory modules + consumer-hardware benchmarks. Measure appropriate refusal for out-of-scope questions using **Openchat-3.6 as judge**. Find a curated ~115-pair set beats a ~564-pair synthetic set on factuality/context/fluency. | They swap **memory stores** with persona fixed in weights; this project swaps **persona adapters *and* visibility sets**. They have **no drift metric**. They *do* evaluate multi-turn behaviour — "Context Retention" over 30 multi-turn interactions (recalling a keyword like the player's name) — but not persona or knowledge-boundary *degradation* across turns. (Corrected 2026-09-11 against the PDF; the earlier note said they had no multi-turn analysis at all, which was wrong.) KBD replaces their judge-based refusal check with an automatic, per-turn, source-attributable metric. This project is framed as a **direct extension** of theirs (overlapping TinyLlama base — one of their three — same consumer-hardware framing, same small-curated-dataset finding adopted as the 600-pair rationale). |
| 2 | **Wang et al.** — *Model Fusion with Multi-LoRA Inference* (arXiv:2509.24229) | Multi-LoRA serving + **LoRA parameter fusion in game NPC dialogue**. Their "model fusion" is **averaging the weights of LoRA checkpoints saved at successive epochs of one training run** (corrected 2026-09-11 against the PDF; the earlier note said it averaged adapters for the same function across data sources, which is not what the paper describes). | Establishes that LoRA weight merging is **standard prior art** — so this project does **not** claim the `W_blend = α·W_A + (1-α)·W_B` technique. This project interpolates **different personas** with tunable α and asks whether the **epistemic boundary** interpolates or collapses to the union — a question about knowledge leakage, not about serving throughput. |
