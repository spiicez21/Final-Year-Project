...
- [x] Built `training/colab_train.ipynb` — clones the repo (already pushed to `github.com/spiicez21/Final-Year-Project`), installs pinned deps matching what was validated locally, mounts Drive so adapters survive runtime resets, W&B login, then queues: (1) the 8-epoch r=8 run that couldn't finish locally, (2) r=32 to complete the planned rank ablation, (3) a 15-epoch escalation if both still null out on archaic markers. No code changes to `train_adapter.py` needed — same script, T4 is also Turing architecture (same bf16 software-emulation path as the local MX450) but without the thermal ceiling.
  - **Next action is yours:** open the notebook in Colab, select a T4 runtime, run cells top to bottom. Report back the `quick_inference.py` outputs for r8_e8 and r32 and we'll decide from there.
