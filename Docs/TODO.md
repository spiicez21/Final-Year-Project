...
- [x] Ninth gap-fill batch, hand-authored in-session: 17 entries — merchant +5, scholar +5, innkeeper +5, herbalist +2 (`SYN-0941`..`SYN-0957`). 181 hand-authored entries total across nine batches.
...
- [ ] Reach 1,000 total entries with balanced archetype distribution (currently 957, see gap table)
- [ ] Build 50-entry stress test corpus (`data/processed/stress_test_corpus.json`) — not started

### Archetype gap (current 957 entries vs. spec target)
...
Merchant/scholar/innkeeper still the biggest gaps, herbalist nearly done (206 entries away from balanced 1,000 overall). Nine hand-authored batches in (181 entries total) — same open question as before: keep grinding batches, or call ~950-970 balanced entries good enough and move to Phase 3 sooner (a judgment call for you, not a technical blocker).
